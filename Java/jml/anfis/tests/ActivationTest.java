...
public class ActivationTest {
    ...
    public double runFromGivenLayer(int layerId, double[] layerVal) {
        double[] inputs = {0.148, 0.125, 0.282};
        double[] coefs = {-2.2801945586853822, 19.941009511195354, -3.5847557414874487, 0.3722705939255775, 0.6359358885823828, 0.0447299699613452, 0.45648985718171237, -0.45301498504638};
        double[] activations;
        double[] rule = new double[2];
...
//            System.out.println("param2=" + anfis.getActivationParamVal(inputIdx, paramIdx) + "; F2=" + func_val2);
            ...
            System.out.println("Gradient = " + activationList[inputIdx].getParamDelta()[paramIdx] + "; stepVal=" + stepVal);
            assertTrue(Math.abs(activationList[inputIdx].getParamDelta()[paramIdx] - stepVal) < delta);
            ...
            int targetElemIdx = 1; // index of the gradient element (or element in the layer)
            layerValues1[targetElemIdx] = layerValues[targetElemIdx] + eps;
            layerValues2[targetElemIdx] = layerValues[targetElemIdx] - eps;

            double output1 = runFromGivenLayer(startLayerId, layerValues1);
            func_val1 = Math.pow(B[0][exampleNo] - output1, 2) / 2.0;

            double output2 = runFromGivenLayer(startLayerId, layerValues2);
            func_val2 = Math.pow(B[0][exampleNo] - output2, 2) / 2.0;

            double s2 = (func_val1 - func_val2) / (2 * eps);
            System.out.println("func_val1="+func_val1+"; func_val2="+func_val2+"; s2 = " + s2);
            */
